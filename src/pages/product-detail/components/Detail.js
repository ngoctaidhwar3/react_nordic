...
function Detail({ product, dispatch, email, cart }) {
  ...
  const categories_name = Object.keys(categories)
    .filter((key) => key.includes("_name"))
    .map((key) => [categories[key]]);
  const categories_path = Object.keys(categories)
    .filter((key) => key.includes("_path"))
    .map((key) => [categories[key]]);
  ...
  return (
    <Fragment>
      {categories_name.map((item, index) => {
        return index !== categories_name.length - 1 ? (
          <span key={index}>
            <Link to={`/${item.path}`}>{item[0]}</Link> &gt;{" "}
          </span>
        ) : (
          ...
          <div className="container">
            <div className="wrapper row">
              <div className="preview col-md-6">
                <div className="preview-pic tab-content">
                  <div className="tab-pane active" id="pic-1">
                    <img
                      alt=""
                      src={"https://media3.scdn.vn" + product.images[imgIndex]}
                    />
                  </div>
                </div>
              ...
              <div className="details col-md-6">
                ...
                {product.required_options.search("kich_thuoc") !== -1 ? (
                  <h5 className="sizes">
                    kích cỡ:
                    <button
                      className="size"
                ...
                {product.final_price === product.price ? (
                  <h4 className="price">
                    {Number(product.final_price).toLocaleString()}đ
                  </h4>
                ) : (
                  <h4 className="price">
                    {Number(product.final_price).toLocaleString()}đ{" "}
                ...
                {product.required_options.search("mau_sac") !== -1 ? (
                  <h5 className="colors">
                    màu sắc:
                    <button
